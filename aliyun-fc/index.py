# -*- coding: utf-8 -*-
"""
aliyun-fc/index.py —— 矿业新闻日报 · 轻后端（仅 /api/qa + /api/health）
部署目标：阿里云函数计算 FC（Python 3.10 HTTP 触发器）

设计目标：
  - 无状态：不读任何数据文件，前端把「问题 + 本地已检索到的相关新闻」一起 POST 过来，
    本函数只负责调 DeepSeek 生成答案（key 存为函数环境变量，不落代码）。
  - 国内访问最稳：阿里云 FC 国内节点，同事访问基本不会被墙。
  - 免 key 共用：key 放在阿里云后台，同事打开网页即可直接用 AI，不用各自输 key。

入口：阿里云 FC 调用 handler(event, context)
event 关键字段（HTTP 触发器）：
