/**
 * 2026-09-21 矿业公司情报板块 运行时渲染回归（jsdom）
 * 覆盖：① fetch 根目录 company_news.json 并渲染 32 张公司卡 + 概要 chips
 ...
 *       ⑤ updateActiveSection 滚动高亮列表含 companySection
...
setTimeout(() => {
  try {
    ...
    // 1) 运行时渲染：32 张卡 + 概要 chips
    const cards = document.querySelectorAll('#companyList .co-card');
    check('渲染出 32 张公司卡', cards.length === 32, 'got ' + cards.length);
    ...
    check('coCount 显示「32 家」', coCount.indexOf('32') !== -1, coCount);
    const items = document.querySelectorAll('#companyList .co-item');
    check('渲染出公司披露条目', items.length > 100, 'got ' + items.length);
    // 抽查：首张卡含公司名 + 链接 href 指向 cninfo/sec
    const firstCard = cards[0];
    const nameOk = firstCard && firstCard.querySelector('.co-name') && firstCard.querySelector('.co-name').textContent.length > 0;
    check('首卡含公司名', !!nameOk);
    const linkOk = firstCard && firstCard.querySelector('.co-link') && /^https?:\/\//.test(firstCard.querySelector('.co-link').getAttribute('href') || '');
    check('首卡链接为 http(s) 外链', !!linkOk);

    // 2) 豁免：refreshSectionVisibility 不隐藏 companySection（默认视图）
    ...
    // 5) 滚动高亮列表含 companySection
    const hasCo = (typeof window.updateActiveSection === 'function');
    check('updateActiveSection 函数存在', hasCo);
    // 直接验证 sections 数组含 companySection（通过调用后 toc active 是否可能落在它）
