...
> 筛选 chip、日期徽章、`--tag-*` 徽章、`.rr-type` 等，它们的实际高度为
...
5. 禁止同一层级出现两种间距值
6. 禁止渐变、发光、装饰性动效
   > **v3 例外（已撤销 · 2026-09-11）**：原拟保留「问」tab 的渐变发光球作 AI 视觉区分；经移动端优化评审，用户确认其"别扭"并选择与其它 tab 一致的平铺样式（单一品牌色、无渐变/发光/脉冲）。据此例外撤销——全站禁止渐变/发光/装饰动效，含「问」tab。
