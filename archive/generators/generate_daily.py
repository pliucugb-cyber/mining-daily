...
# ============================ ⚠️ 已废弃（DEPRECATED） ============================
# 2026-09 第 3 批工程整洁度检查确认：本脚本是 8 月底的遗留物，**不要使用**。
#
# 判定依据：
#   1. 硬编码了他人机器的绝对路径 C:\Users\windows\WorkBuddy\2026-08-25-21-20-31\...，
#      在本机运行第 13 行就会 FileNotFoundError，根本跑不起来。
#   2. 全仓检索（含 06:00 自动化用的 .workbuddy 记忆、所有 .py/.md/.json/.html）
#      **没有任何代码调用它**，仅有的两处提及都是注释/审计文档。
#   3. 它对应的是 2026-08-27 的单日产物，早已被 generate_YYYYMMDD.py 系列取代。
#
# 为什么保留而不是删除：git 历史里它曾被当作「生成器模板」顺手同步过文字，
# 直接删会让后续翻历史的人误以为从未存在过。保留文件 + 本弃用声明即可。
#
# 现在的正确做法：每日重建页面请用当日的 generate_YYYYMMDD.py，
# 公共骨架见 generate_common.py。
# ==============================================================================
import re
import sys
import json
