...
数据来源：44 家矿业龙头「官方网站新闻栏目」（非 cninfo/SEC 公告），按三组划分：
  - 国内 A股（29 家）：各公司官网新闻列表页；其中 5 家原死站/JS 外壳站（云南铜业/云铝股份/湖南黄金/
    ...
    （2026-09-26 晚按用户要求删除 盐湖股份 / 永兴材料 / 株冶集团 三家，原 47 家；新增剔除只需加到下方 EXCLUDE 常量，下一轮 fetch 自动过滤，无需手工改 company_news.json 副本）
...
EXCLUDE = ['盐湖股份', '株冶集团', '永兴材料']  # 移除名册（配置化删除）：加到这里即永久剔除；fetch 采集后过滤，preflight 据此外校验
...
def main():
    ...
    companies = [c for c in companies if c.get('name') not in EXCLUDE]
