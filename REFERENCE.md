...
### 42.19 矿业公司动态（v4：跨列区块 + 右 sticky 侧栏，2026-09-23 二改版）

**定位（必留）**：全站独立主区块 `#companySection`（左侧目录「🏢 矿业公司」入口，`data-target="companySection"`），**v4 形态 = 跨满 `news-grid` 两列的独立区块：左＝新闻流（`minmax(0,1fr)`），右＝ `272px` sticky 侧栏（`#coSearch` 搜索框 + `#coFilters` 矿种 chips + `#coNav` 公司导航 + `#coStat` 统计）**。v3 的「区块内左侧公司列」已废除 —— 原因是它与站点左侧悬浮目录导航叠加成「三重左栏」、把新闻流挤到中间，而右栏热榜滚出视口后留下一大片空白（`.col-rail` 因 `.news-grid` 的 `align-items:start` 使元素高度=内容高，`position:sticky` 只在第一行行高（＝`.col-main` 高）内滚动，到本区块所在的第二行时已滚走）。`#coNav` 按矿种分组列出全部 32 家公司（含「全部公司」入口；**无数据公司折进 `.co-nav-empty-t` 折叠组**，默认收起、点公司名直达官网）；`#companyList` 渲染 `.co-item` 新闻流。点击公司项 / 点条目来源名 `.co-src` → 新闻流收敛到该公司 + 头部显示公司名 + 写入 hash `#co=公司名`（history.replaceState）；`hashchange` 监听实现深链直达。移动端（≤1100px）`#coNavSel`（`<select>`，**按矿种 `<optgroup>` 分组**，选项 = 公司数 + 1）替代公司导航。数据来自根目录 `company_news.json`，**纯前端运行时 `fetch` 渲染**（app.js `renderCompanySection()` IIFE），**不引入生成侧 `TAG_COMPANY`**——与 §42.14/§42.15/§42.16/§42.17「前缀区静态物 + 运行时渲染」套路一致。视觉语言用独立 `.co-*` 类，**不继承**新闻已读/收藏态，矿种标记 `.co-cat` **不可**挪用红涨绿跌色。

**新闻流展示规则（v4 新增，必留）**：① **按日期分组** —— `.co-day`（`.co-day-h` 分组头「今天／昨天／N 天前／未标注日期 · YYYY-MM-DD 周X」+ `.co-day-n` 条数）、`.co-day-items` 装当天条目；② **宽屏两栏网格** —— `@media(min-width:1400px)` 下 `.co-day-items{grid-template-columns:1fr 1fr}`（1080px 级窗口仍单栏，避免过窄）；③ **时间范围切换** —— `#coFeedHead` 内 `.co-range` 三个按钮 `data-r="30|90|0"`，**默认 `range=90`（近 90 天）**：数据里混有 2025 年旧链接与 29 条 `d:""` 无日期条目，若默认全量平铺，「全站最新动态」观感即「垃圾堆」；`passRange()` 中 `range===0` 放行全部，**`d===""` 的条目只在「全部」下出现**；④ **分页** —— `PAGE=60`，`#coMoreWrap`/`#coMore`（「加载更多」）+ `#coMoreHint`（「已显示 x / y 条」），`total>shown` 时显示；任何筛选动作（切公司／切矿种／改搜索／切范围）都**重置 `shown=PAGE`**；⑤ 头部 `.co-feed-sub` 文案固定为「显示 x / y 条」（**不要**退回 v3 的裸条数）。排序：`d` 降序，`d===""` 置底。

**采集裁定（用户拍板 + 2026-09-23 修正）**：① 数据来源 = 各公司**官网新闻栏目**（非 cninfo/SEC 公告）；② 范围 = 国内 A 股 25 家 + 海外 7 家共 32 家；③ 海外英文标题经 MyMemory 译中（保留 `t_en` 原文）；④ 新闻优先过滤（口径「适中＝事件类 + 行业技术观察」）：`NON_NEWS_HREF`/`NON_NEWS_TITLE`/`NON_NEWS_TITLE_KW` 丢栏目/介绍/招聘/党建/矿山项目页（如紫金 `/global/program-detail-*` 矿山页被干掉），`NEWS_VERB`/`NEWS_URL_HINT` 守事件/技术观察；日期探测 = URL 内嵌日期（`_date_from_url`，如 `t20260916`）+ 锚点前后各 5000 字符文本，**`require_date` 默认 False**（中文站日期常离标题较远/仅在 URL，强制日期会误杀真实事件新闻，故保留 `d:''` 置底）。采集层 `fetch_company.py` 现已**全站静态 urllib 抓取**：本机 Chrome headless 在当前环境会卡死（>120s，且 profile 清理触发沙箱拦截），故废弃 `method='chrome'`、统一 `method='html'`；海外站经 `effective_proxy()` 运行时发现存活的本机 HTTP 代理端口（企业网代理端口会漂移，不再写死，带 60s 漂移重验）取外网，国内站直连兜底。**已知局限（环境所致，非代码缺陷）**：海外 7 家中仅 Teck、Albemarle 的新闻列表是静态可抽；Newmont/Barrick/Agnico/Freeport 的新闻页是 ASPX/JS 渲染外壳（静态 HTML 无文章链接、无 RSS）、Southern Copper 经代理超时不可达 → 这 5 家在本环境只能为空（UI 显示「数据暂缓」），需换源/服务端抓取才能补。原始 HTML 落盘 `tmp/co_cache/<slug>.html` 24h 复用，支持 `--only 名1,名2` 单家重采与 `--force` 强刷。

**容器与顺序（生成侧必留，v4 已变更）**：`#companySection` **已移出 `.col-main`，成为 `.news-grid` 的直接子元素**，并靠 CSS `#companySection{grid-column:1/-1}` 跨满两列；`#installGuideSection` 紧随其后（同样移出 `.col-main`，保持「公司动态 → 安装指引」的原有相对顺序）。DOM 顺序为 `… .col-main → #companySection → #installGuideSection → <aside class="col-rail">`，**因此 `.col-rail` 必须由 CSS 显式放回第一行第二列**：`@media(min-width:1101px){.news-grid>.col-rail{grid-column:2;grid-row:1}}`（该规则与 `#companySection{grid-column:1/-1}` 一并写在 `#companySection > style` 内）—— 否则 grid 自动放置会把热榜挤到页面最底部。`#eventCalendar`、`aside.fav-toc`、`#archivedFavSection` **仍留在 `.col-main` 内**（前者 `EC_ENABLED=false` 常隐、后两者仅收藏视图用），不必随迁。区块内含内联 `<style>`（`.section-title.co` + `.co-*` 全套），样式随区块常驻。

**数据契约（`company_news.json`）**：`{updated_at, companies:[{name, code, sector, region("CN"|"NA"), exchange, home, news_url, method("html"|"err"), stale:bool, items:[{t(中), t_en?(英原文), d("YYYY-MM-DD" 或 ""), u, s?(摘要)}]}], counts:{domestic,foreign,total,items}}`。**无** `lv` 重要性分级（v1 已废）。UI 用 `.co-cat` 显示矿种（非红涨绿跌色）。

**前端句柄（app.js）**：`window.renderCompanySection`（IIFE，加载即调 + `DOMContentLoaded` + `setTimeout 1500` 兜底）；`fetch('company_news.json',{cache:'reload'})` → 渲染 `#coNav`（`.co-nav-all`「全部公司」+ 按 `sector` 分组的 `.co-nav-item`，**均为 `<button type="button">` 可键盘操作**；无数据公司进 `#coEmptyToggle` / `#coEmptyBox` 折叠组，`.co-nav-item.empty` 带 `data-home`、点名称开官网）、`#coFilters`（`.co-fchip` 矿种，含「全部」）、`#coNavSel`（移动端 select，`<optgroup>` 分组）、`#coSearch`、`#coStat`、`#coFeedHead`（`.co-feed-name` 当前公司名 + `.co-feed-sub`「显示 x / y 条」+ `.co-fh-right` 内 `.co-range` 与 `.co-feed-link`「官网↗」）、`#companyList`（`.co-day` → `.co-day-items` → `.co-item`＝`.co-dot` + `.co-title` http(s) 外链 + `.co-cat` 矿种 + `.co-meta`（`.co-src` 可点公司 · 日期 · `海外`/`数据暂缓`）+ `.co-summary` 摘要）、`#coMoreWrap`/`#coMore`/`#coMoreHint`（分页）、`#coCount`（「N 家」）。`setHash`/`selectCompany` 用 `#co=encodeURIComponent(name)` + `history.replaceState`；`hashchange` 监听选公司。**测试句柄 `window.__mdCo`**：`state()`（range/shown/公司数/条目数/渲染数/分组数/moreVisible）、`setRange()`、`setQuery()`、`select()`。

**豁免（关键）**：`refreshSectionVisibility()`（§42.8）「静态说明区跳过」白名单**必须含 `companySection`**（否则被「无 `.news-item` 即 `display:none`」误隐藏；公司条目类是 `.co-item`）。`switchView` 的 `targetId` 映射与 `updateActiveSection` 的 `sections` 列表**都必须含 `companySection`**。
...
**回退指纹**：① `#companySection` 静态容器缺失；② 左侧目录无「🏢 矿业公司」入口；③ 公司条目用 `.news-item` 而非 `.co-item`（被显隐闸门误隐藏）；④ `refreshSectionVisibility` 白名单漏 `companySection` → 默认视图整块不可见；⑤ `fetch` 路径非 `'company_news.json'`（线上 404）；⑥ `deploy_pages.py` 的 `OPTIONAL` 漏 `company_news.json` 或 `sw.js` 的 `DATA_FILES` 漏它 → 数据不发布/缓存中毒；⑦ 绿涨红跌色被挪用；⑧ 仍出现 cninfo/SEC 公告或矿山项目页（违背采集裁定）；⑨ **`#companySection` 退回 `.col-main` 内部（变回单列窄区块）或丢失 `grid-column:1/-1`**；⑩ `#coNav`/`#coNavSel` 任一缺失或 hash 路由 `#co=` 失效（深链断裂）；⑪ 移动端 select 选项 ≠ 公司数 + 1；⑫ **`.col-rail` 的 `grid-column:2;grid-row:1` 规则丢失 → 热榜被挤到页面最底部**；⑬ 搜索框／矿种 chips／公司导航任一**不在 `.co-side` 内**（退回 v3 顶部 `.co-bar` 横条形态）；⑭ **新闻流退回「无分组的 199 条平铺」**（`.co-day` 缺失）或**默认 `range` 不是 90**（旧闻混入「全站最新动态」）；⑮ `window.__mdCo` 缺失（回归测试句柄）。

**闸门**：`node test_company_section.js`（jsdom，**64 项**，覆盖 v4：跨列区块 + `.co-side` 收纳 + `.col-rail` 显式定位 + 默认近 90 天/分页 60/日期分组 + 切「全部」并连续「加载更多」渲染全量 + 未收录折叠组展开/收回 + 公司收敛 + hash 路由 + 搜索 + optgroup select + 豁免显隐 + `switchView('company')` 隔离 + 无 JS 错误）。改过 `@media`/网格断点 → 仍须补**真实 Chrome** 探针（jsdom 不评估 `@media`；**跨列 + sticky + 两栏网格三个形态只能在真浏览器里肉眼确认**）。**注意**：本测试用本地 `company_news.json` 作 mock，范围/条数断言按 `updated_at` 与 `d` 现算（不写死数字），但**默认渲染数 = min(60, 近 90 天条数）**依赖数据分布，重采后若范围口径变化需同步核对。
...
| `node test_company_section.js` | **64 PASS** | 矿业公司情报板块（v4 跨列区块 + 右 sticky 侧栏）：跨列布局（companySection/installGuide 为 `.news-grid` 直接子级、col-rail 显式定位）+ `.co-side` 收纳搜索/chips/公司导航 + 默认近 90 天 + 分页 60 + 日期分组 + 切「全部」并连续「加载更多」渲染全量 199 条 + 未收录公司折叠组 + 点公司收敛 + hash 路由 `#co=` 深链 + 搜索过滤 + 移动端 `#coNavSel` optgroup + 豁免显隐 + `switchView('company')` 视图隔离 + 无 JS 错误（§42.19） |
