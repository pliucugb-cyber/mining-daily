...
**部署必留**：① `deploy_pages.py` 的 `OPTIONAL` 列表**必须含 `company_news.json`**；② `sw.js` 的 `DATA_FILES` 列表**必须含 `BASE+'company_news.json'`**；③ `sw.js` `CACHE_NAME` 与 `index.html` `build-version` 须同步 bump（本次 `20260923-2200`）。
