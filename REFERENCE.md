...
### 42.19 矿业公司动态（v5：长度统一 + 去矿种 + 链接修复 + 导航整理，2026-09-23 三改版）

**定位（必留）**：全站独立主区块 `#companySection`（左侧目录「🏢 矿业公司」入口，`data-target="companySection"`），v5 形态 = 跨满 `news-grid` 两列的独立区块：左＝新闻流（`minmax(0,1fr)`），右＝ `288px` sticky 侧栏（`#coNav` 公司导航（按条目数降序 + 计数徽标 + 内部滚动）+ `#coStat` 统计）。**v5 已废除矿种维度**：删除 `#coFilters` 矿种 chips、`.co-cat` 矿种标记、移动端 `<optgroup>` 矿种分组（紫金不全是铜、中铝不全是铝，分矿种会误导用户）；v3 的「区块内左侧公司列」同样已废除。`#coNav` 按**条目数降序**列出全部 32 家公司（含「全部公司」入口；无数据公司折进 `#coEmptyToggle`/`#coEmptyBox` 折叠组，默认收起、点公司名直达官网）；`#companyList` 渲染 `.co-item` 新闻流。点击公司项 / 点条目来源名 `.co-src` → 新闻流收敛到该公司 + 头部显示公司名 + 写入 hash `#co=公司名`（history.replaceState）；`hashchange` 监听实现深链直达。移动端（≤1100px）`#coNavSel`（`<select>`，**flat 选项 = 公司数 + 1，仅一个「暂未收录」optgroup**）替代公司导航。数据来自根目录 `company_news.json`，**纯前端运行时 `fetch` 渲染**（app.js `renderCompanySection()` IIFE），**不引入生成侧 `TAG_COMPANY`**。视觉语言用独立 `.co-*` 类，**不继承**新闻已读/收藏态，且**不得**挪用红涨绿跌色（v5 复用既有 token `--ink-900`/`--surface-2`，不新增 `--ink-800`/`--surface-1` 这类未定义变量）。

**新闻流展示规则（v5 新增长度统一 + 链接修复，必留）**：① **按日期分组** —— `.co-day`（`.co-day-h` 分组头「今天／昨天／N 天前／未标注日期 · YYYY-MM-DD 周X」+ `.co-day-n` 条数）、`.co-day-items` 装当天条目；② **宽屏两栏网格** —— `@media(min-width:1400px)` 下 `.co-day-items{grid-template-columns:1fr 1fr}`；③ **时间范围切换** —— `#coFeedHead` 内 `.co-range` 三按钮 `data-r="30|90|0"`，**默认 `range=90`**；`passRange()` 中 `range===0` 放行全部，`d===""` 的条目只在「全部」下出现；④ **分页** —— `PAGE=60`，`#coMoreWrap`/`#coMore`/`#coMoreHint`，`total>shown` 时显示，任何筛选动作**重置 `shown=PAGE`**；⑤ **长度统一（v5 核心）** —— `t` 字段长短悬殊（4 字～1055 字，21 条 ≥120 字、29 条 ≤15 字），`cleanT(t)` 清洗「序号+年月」前缀/尾随日期/不间断空格后，`splitTB(t)` 在首个句末标点（≤64 字）处拆为 **标题**（`head`，2 行 clamp）+ **正文**（`body`，仅当 body 超出 2 行折叠阈值才渲染，2 行 clamp + 「展开全文」`.co-exp` 切换 `.co-item.open` 显全）。短标题（≤15 字）只显示标题，不出现折叠/展开；⑥ **链接修复（v5 核心）** —— 26/199 条 URL 在 `company_news.json` 里含字面 `&amp;`（tlys/shenhuo/teck/albemarle），旧版 `esc()` 再转义一次成 `&amp;amp;` 致链接打不开。`dec(u)` 在 `esc()` **之前**解码 HTML 实体（`&#x..;`/`&#..;`/`&lt; &gt; &quot; &apos; &nbsp;`、`&amp;` 置最后避免二次解码），主链接 = 解码后 URL 或 `searchUrl(公司名+标题)` 兜底；外链统一 `rel="noopener noreferrer"`，并展示目标主机 `.co-host`。排序：`d` 降序，`d===""` 置底。

**采集裁定（未变）**：① 数据来源 = 各公司**官网新闻栏目**；② 范围 = 国内 A 股 25 家 + 海外 7 家共 32 家；③ 海外英文标题经 MyMemory 译中（保留 `t_en` 原文）；④ 新闻优先过滤（口径「适中＝事件类 + 行业技术观察」），`require_date` 默认 False（中文站日期常离标题远/仅在 URL，强制日期误杀真实事件新闻，故保留 `d:''` 置底）。采集层 `fetch_company.py` 现已**全站静态 urllib 抓取**（本机 Chrome headless 在当前环境卡死，统一 `method='html'`）；海外站经 `effective_proxy()` 运行时发现存活本机 HTTP 代理端口取外网。**已知局限**：海外 7 家中仅 Teck、Albemarle 静态可抽；Newmont/Barrick/Agnico/Freeport 是 ASPX/JS 渲染外壳、Southern Copper 经代理超时 → 这 5 家只能为空（UI 显示「数据暂缓」）。原始 HTML 落盘 `tmp/co_cache/<slug>.html` 24h 复用，支持 `--only` 单家重采与 `--force` 强刷。**链接实体坑（v5 已修前端解码，采集侧仍应避免落 `&amp;` 字面）**。
...
**部署必留**：① `deploy_pages.py` 的 `OPTIONAL` 列表**必须含 `company_news.json`**；② `sw.js` 的 `DATA_FILES` 列表**必须含 `BASE+'company_news.json'`**；③ `sw.js` `CACHE_NAME` 与 `index.html` `build-version` 须同步 bump（本次 `20260923-2310`）。

**回退指纹**：① `#companySection` 静态容器缺失；② 左侧目录无「🏢 矿业公司」入口；③ 公司条目用 `.news-item` 而非 `.co-item`（被显隐闸门误隐藏）；④ `refreshSectionVisibility` 白名单漏 `companySection` → 默认视图整块不可见；⑤ `fetch` 路径非 `'company_news.json'`（线上 404）；⑥ `deploy_pages.py` 的 `OPTIONAL` 漏 `company_news.json` 或 `sw.js` 的 `DATA_FILES` 漏它 → 数据不发布/缓存中毒；⑦ 红涨绿跌色被挪用（或 `.co-*` 出现未定义 token 如 `--ink-800`/`--surface-1`）；⑧ 仍出现 cninfo/SEC 公告或矿山项目页（违背采集裁定）；⑨ `#companySection` 退回 `.col-main` 内部（变回单列窄区块）或丢失 `grid-column:1/-1`；⑩ `#coNav`/`#coNavSel` 任一缺失或 hash 路由 `#co=` 失效（深链断裂）；⑪ 移动端 select 选项 ≠ 公司数 + 1 或 **仍按矿种 `<optgroup>` 分组**（v5 应为单「暂未收录」optgroup）；⑫ `.col-rail` 的 `grid-column:2;grid-row:1` 规则丢失 → 热榜被挤到页面最底部；⑬ **矿种维度复活**（`#coFilters`/`.co-fchip`/`.co-cat`/矿种 `<optgroup>` 任一重现）；⑭ 新闻流退回「无分组的 199 条平铺」（`.co-day` 缺失）或默认 `range` 不是 90；⑮ **长度不统一**（长 `t` 仍裸平铺、无 `.co-body`/`.co-exp` 折叠展开，或短标题出现空折叠）；⑯ **链接仍双重转义**（href 含 `&amp;amp;` 或解码后打不开）+ 外链缺 `rel="noopener noreferrer"`/`.co-host`；⑰ **`forcedAll` 失效** → 选中「全部条目都在范围外」的公司时新闻流空列表（应自动放宽到全部）；⑱ `#coNav` **不再按条目数降序**或缺失计数徽标；⑲ `window.__mdCo` / `window.__mdCo.helpers` 缺失（回归测试句柄）。

**闸门**：`node test_company_section.js`（jsdom，**100 项**，覆盖 v5：跨列区块 + 288px 侧栏 + 无矿种维度 + 长度统一（长文拆标题/正文 + 2 行 clamp + 展开全文）+ 链接 `dec()` 解码 + `rel=noreferrer` + `.co-host` + 默认近 90 天/分页 60/日期分组 + 切「全部」并连续「加载更多」渲染全量 + 未收录折叠组 + 公司收敛 + **forcedAll 自动放宽** + hash 路由 + 搜索 + **flat select（单 optgroup）** + 按条目数降序导航 + 豁免显隐 + `switchView('company')` 隔离 + 无 JS 错误）。改过 `@media`/网格断点 → 仍须补**真实 Chrome** 探针（jsdom 不评估 `@media`；跨列 + sticky + 两栏网格三形态只能在真浏览器肉眼确认）。
...
| `node test_company_section.js` | **100 PASS** | 矿业公司动态（v5：长度统一 + 去矿种 + 链接修复 + 导航整理）：跨列布局（companySection/installGuide 为 `.news-grid` 直接子级、col-rail 显式定位）+ 288px 侧栏无矿种维度 + 默认近 90 天 + 分页 60 + 日期分组 + 长 t 拆标题/正文 2 行 clamp + 展开全文 + 链接 `dec()` 解码 + `rel=noreferrer` + `.co-host` + 切「全部」并连续「加载更多」渲染全量 + 未收录公司折叠组 + 点公司收敛 + **forcedAll 自动放宽** + hash 路由 `#co=` 深链 + 搜索过滤 + 移动端 `#coNavSel` flat 单 optgroup + 按条目数降序导航 + 豁免显隐 + `switchView('company')` 视图隔离 + 无 JS 错误（§42.19） |
