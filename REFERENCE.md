...
**定位（必留）**：全站独立主区块 `#companySection`（左侧目录「🏢 矿业公司」入口，`data-target="companySection"`），v6 形态 = 跨满 `news-grid` 两列的独立区块：左＝新闻流（`minmax(0,1fr)`），右＝ `288px` sticky 侧栏（`#coNav` 公司导航（国内/海外分组 + `rank` 升序 + 计数徽标 + 内部滚动）+ `#coStat` 统计）。**v5 已废除矿种维度**（v6 沿用）：删除 `#coFilters` 矿种 chips、`.co-cat` 矿种标记、移动端 `<optgroup>` 矿种分组（紫金不全是铜、中铝不全是铝，分矿种会误导用户）；v3 的「区块内左侧公司列」同样已废除。`#coNav` 按**国内/海外两组、组内按市值·知名度 `rank` 升序**列出「有内容」公司（含「全部公司」入口；无数据公司统一收进 `#coEmptyToggle`/`#coEmptyBox` 折叠组——国内/海外两组只列 `itemCount>0` 的活公司，空壳不重复出现，默认收起、点公司名直达官网）；`#companyList` 渲染 `.co-item` 新闻流。点击公司项 / 点条目来源名 `.co-src` → 新闻流收敛到该公司 + 头部显示公司名 + 写入 hash `#co=公司名`（history.replaceState）；`hashchange` 监听实现深链直达。移动端（≤1100px）`#coNavSel`（`<select>`，`option = 公司数 + 1`，**三个 optgroup：国内公司 / 海外公司 / 暂未收录**）替代公司导航。数据来自根目录 `company_news.json`，**纯前端运行时 `fetch` 渲染**（app.js `renderCompanySection()` IIFE），**不引入生成侧 `TAG_COMPANY`**。视觉语言用独立 `.co-*` 类，**不继承**新闻已读/收藏态，且**不得**挪用红涨绿跌色（v5 复用既有 token `--ink-900`/`--surface-2`，不新增 `--ink-800`/`--surface-1` 这类未定义变量）。
