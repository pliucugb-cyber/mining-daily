...
数据源：mining_news.json（109 条，今日新增 10 条）+ price_history_detail.json（15 品种日 K）
报告日期 2026-09-09（国内 SHFE/上金所最新收盘 09-08；LME 09-09 早盘）
"""
...
ma_text = '\n'.join(fmt_bullet(n) for n in recent_items('并购与投资')) or '今日暂无新增并购/投资类公告（巨潮接口 09-09 返回 504，次日补抓）。'
...
report = """**行情：**
国内 09-08 收盘——{dom_up}领涨，{dom_down}走弱；上海金 Au99.99 报 953.12 元/克、白银 Ag(T+D) 16,125 元/千克同步小幅上涨。LME 09-09 早盘（美元/吨）：{lme_up}偏强，{lme_down}走弱；锡今日报价暂未更新，沿用 9-8 收盘 54,862。（各品种完整报价见上方「金属价格」卡片）
...
**矿权市场：**
- 09-09 当日新增 2 宗：辽宁岫岩县席大岭金多金属矿普查探矿权转让公示（转让方晶瑞矿业→受让方南洋金融信息服务）；新疆托里县成吉思汗山安山岩矿二区普查探矿权公开挂牌出让（塔城，安山岩，0.1049 平方千米）。
- 09-08 集中公示 4 宗：山东乳山市金青顶外围金矿普查探矿权挂牌出让结果公示（中标人山东金洲矿业集团，金矿，10.830 平方千米）；四川阿坝州马尔康市沙普金矿勘查探矿权挂牌出让公告（金矿，40.9006 平方千米）；内蒙古阿巴嘎旗哈达特陶勒盖银铅锌矿平面垂直投影深部探矿权协议出让公示（受让方内蒙古鑫益矿业，铅矿，5.5428 平方千米）；内蒙古赤峰宁城陈家杖子金矿采矿权垂直投影深部探矿权协议出让公示（受让方宁城宏丰实业，金矿，1.2686 平方千米）；内蒙古乌拉特前旗多金矿业德尔斯台沟等铁矿深部及夹缝区域勘查探矿权协议出让公示（铁矿，11.7923 平方千米）。
- 09-04 已公示、9-09 进入版块整合采集。同一公告在 cninfo / szse 间高度重叠的去重后只留 1 条，避免冗余。
""".strip().format(
    dom_up='、'.join(_shfe_up) or '全线偏弱',
    dom_down='、'.join(_shfe_down) or '—',
    lme_up='、'.join(_lme_up) or '—',
    lme_down='、'.join(_lme_down) or '—',
